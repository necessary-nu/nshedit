...
///
// TODO(nshterm): TERMINFO, TERMINFO_DIRS and $HOME are read straight from the
// environment with no set-uid check, so a set-uid consumer can be aimed at an
// attacker's terminfo entry — which is attacker-controlled escape sequences.
// ncurses guards exactly this in `use_terminfo_vars()`. libedit already routes
// TERM through `secure_getenv`, so today the terminal *type* is guarded and the
// *database* is not. See plan node `nshterm-env-guard`.
//
