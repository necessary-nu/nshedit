...
// `wchar_t`, `el_rfunc_t` and `CFile` are C spellings; see [`super`].
...
/// [`wchar_t`] so the three members render `const wchar_t *` rather than
/// `const uint32_t *`.
///
...
pub type LineInfoWide = LineInfoGen<wchar_t>;
...
pub type HistEventWide = HistEventGen<wchar_t>;
...
/// type as [`nshedit::histedit::ElRfuncT`]. cbindgen prints the name it sees,
/// and the name `EditLine` resolves in the generated header to
/// [`super::handles::EditLine`], the incomplete type the C declares.
pub type el_rfunc_t = unsafe extern "C" fn(*mut EditLine, *mut wchar_t) -> c_int;
...
const _: () = {
    ...
    fn rfunc(x: el_rfunc_t) -> nshedit::histedit::ElRfuncT {
        x
    }
