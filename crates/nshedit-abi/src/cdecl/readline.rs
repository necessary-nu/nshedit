...
pub type KeymapEntryArrayAlias = [KeymapEntryAlias; 256];
...
const _: () = {
    fn hist_entry(x: HistEntryAlias) -> HistEntry {
        x
    }
    fn history_state(x: HistoryStateAlias) -> HistoryState {
        x
    }
    fn keymap_entry(x: KeymapEntryAlias) -> KeymapEntry {
        x
    }
